...
@OnlyIn(Dist.CLIENT)
public class PlayerTabOverlayGui extends net.minecraft.client.gui.overlay.PlayerTabOverlayGui {
	private static final Ordering<NetworkPlayerInfo> ENTRY_ORDERING = Ordering.from(new PlayerTabOverlayGui.PlayerComparator());
	public static final String prefixRemote = TextFormatting.GRAY + " [R] " + TextFormatting.RESET;
	public static final String prefixLocal = TextFormatting.GRAY + " [L] " + TextFormatting.RESET;
	...
	public ArrayList<NetworkPlayerInfo> remotePlayerInfo = new ArrayList<NetworkPlayerInfo>();
	...
	public void setVisible(boolean visible) {
		...
		if (visible && !this.visible) {
			long mil = Util.milliTime();
			if ((mil - this.lastTimeUpdated) >= 1000 * 20 || this.remotePlayerInfo.size() == 0) {
				MessageHandler.INSTANCE.sendToServer(new MessageRequestConnectedPlayers());
				this.lastTimeUpdated = mil;
			}
			this.lastTimeOpened = mil;
		}
	...
	public void render(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
		...
		if(this.remotePlayerInfo.size() > 0) {
			list = this.remotePlayerInfo;
		}
		int i = 0;
		int j = 0;

		for (NetworkPlayerInfo networkplayerinfo : list) {
			int k = this.mc.fontRenderer.getStringWidth(prefixRemote + this.getDisplayName(networkplayerinfo).getFormattedText());
			i = Math.max(i, k);
		...
		for (int i5 = 0; i5 < i4; ++i5) {
			...
			if (i5 < list.size()) {
				...
				String s4 = prefix + this.getDisplayName(networkplayerinfo1).getFormattedText();
	...
	public NetworkPlayerInfo makeFakePlayerNetworkInfo(String uuid, String name, int gametype, int latency) {
		GameProfile gp = new GameProfile(UUID.fromString(uuid), name);
		SPlayerListItemPacket splip = new SPlayerListItemPacket();
		AddPlayerData apd = splip.new AddPlayerData(gp, latency, GameType.getByID(gametype), new StringTextComponent(name));
		NetworkPlayerInfo npi = new NetworkPlayerInfo(apd);
		return npi;
	}
	...
	static class PlayerComparator implements Comparator<NetworkPlayerInfo> {
		...
		public int compare(NetworkPlayerInfo p_compare_1_, NetworkPlayerInfo p_compare_2_) {
			...
			return ComparisonChain.start().compareTrueFirst(p_compare_1_.getGameType() != GameType.SPECTATOR, p_compare_2_.getGameType() != GameType.SPECTATOR)
					...
					.compare(p_compare_1_.getGameProfile().getName(), p_compare_2_.getGameProfile().getName(), String::compareToIgnoreCase).result();
		}
	}
}